...
const TRANSFORMATIONS = [
  { from: 'Тёплая коричнево-бежевая палитра', to: 'Глубокая сине-золотая палитра ночного океана' },
  { from: 'Преимущественно текстовая подача', to: 'Контрастная типографика, карточки и визуальные акценты' },
  { from: 'Компактный первый экран с постером', to: 'Атмосферная композиция с образом лайнера' },
  { from: 'Одна основная CTA', to: 'Основная CTA и дополнительный переход к разбору эпизода' },
  { from: 'Подробности курса раскрываются последовательно по странице', to: 'Ключевые параметры курса видны уже на первом экране' },
  { from: 'Программа представлена текстовыми блоками', to: 'Этапы обучения собраны в таймлайн, а эпизоды — в отдельные карточки' },
  { from: 'Тарифы требуют последовательного чтения', to: 'Три варианта можно сопоставить по единой структуре' },
  { from: 'Информация об авторе встроена в общий поток страницы', to: 'Автору посвящён самостоятельный визуальный раздел' },
]
          ...
          <Reveal delay={120}>
            <div className="flex flex-col gap-3">
              ...
              <BrowserFrame
                ...
                tone="dark"
                chromeLabel="Авторская концепция"
                height={420}
              />
            </div>
          </Reveal>
