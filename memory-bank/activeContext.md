...
## ЗАДАЧА В РАБОТЕ: третий кейс «Мир Кунфу»
**Читай подробный отчёт/план: `Доп/отчёт_кейс3_МирКунфу_планИнтеграции.md` — там полный чек-лист и подводные камни.**
- Архив `pitch-presentation_case.zip` с кейсом «Мир Кунфу» (инвест-питч-дек для документальной медиа-экосистемы).
- Задача: подключить нарративную страницу кейса как `/kunfu` и оживить карточку «Кейс 03» на главной.
- Презентация (питч-дек из 12 слайдов) **УЖЕ опубликована в интернете**; сам питч-дек в проект НЕ переносить. Владелец пришлёт ссылку — подключить её к кнопке «Смотреть результат» на странице кейса.
- Переносить только: `case-study-page.tsx` + кастомный CSS + шрифты (Geist, Noto Serif SC, Fraunces italic/cyrillic-ext).
- Статус: анализ и план готовы; интеграция НЕ начата. Нужно запросить ссылку на презентацию у владельца.
